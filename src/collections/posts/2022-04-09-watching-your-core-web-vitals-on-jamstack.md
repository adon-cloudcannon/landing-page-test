...
Before we move on, take a moment to run a [quick test](https://pagespeed.web.dev/){: target="_blank" rel="noopener noreferrer"} on one of your sites. You’ll see that your site pages are given an aggregate score based on the results for 75% of pages within a URL group. Google’s assumption here is that performance issues in similar pages are likely due to the same underlying problem.

[Lighthouse tests](https://developers.google.com/web/tools/lighthouse){: target="_blank" rel="noopener noreferrer"}&nbsp;are a helpful method to measure your web vitals — you can run Lighthouse tests from Chrome DevTools, via the [Lighthouse Chrome Extension](https://chrome.google.com/webstore/detail/lighthouse/blipmdconlkpinefehnmjammfjpmpbjk){: target="_blank" rel="noreferrer noopener"}, or in the command line:
...
You can also [use Lighthouse programmatically](https://github.com/GoogleChrome/lighthouse/blob/master/docs/readme.md#using-programmatically){: target="_blank" rel="noopener noreferrer"}, as a Node module.
...
* Use a CDN to deploy on the edge. See CloudCannon’s [Jamstack hosting performance comparison](https://cloudcannon.com/community/jamstack-hosting-comparison/){: target="_blank" rel="noopener"}.&nbsp;
* Remove unnecessary JavaScript. Static site generators like [Astro](https://astro.build/){: target="_blank" rel="noopener noreferrer"}&nbsp;and [SvelteKit](https://kit.svelte.dev/){: target="_blank" rel="noopener noreferrer"} (both supported by CloudCannon) are great solutions here, as they give both the speed of static and dynamic interactions of an SPA.
* Minify your CSS, JS, and HTML where possible. Again, your chosen build tools will help you here — Sass allows `style: compressed`, for example, and something like [jekyll-minifier](https://github.com/digitalsparky/jekyll-minifier){: target="_blank" rel="noopener noreferrer"}&nbsp;could be a solid turnkey option for Jekyll users to minify HTML, XML, CSS, JSON, and JavaScript.
...
* Serve optimized images and videos that only load when they’re needed. Formats like AVIF and [WebP](https://developers.google.com/speed/webp/faq){: target="_blank" rel="noopener noreferrer"}&nbsp;images are generally better optimized than legacy image formats — though depending on the image you're serving, in some cases a PNG or SVG might be ideal. Further, [lazy loading](https://developers.google.com/web/fundamentals/performance/lazy-loading-guidance/images-and-video/){: target="_blank" rel="noopener noreferrer"}&nbsp;is a simple change to add into your workflow.
